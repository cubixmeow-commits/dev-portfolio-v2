...
/**
 * Fixture dry-run for Money Collection Cookbooks.
 *
 ...
 *
 * Usage: php scripts/money_collection_dry_run.php
 */

require_once __DIR__ . '/../database/seeds/money_helpers.php';
require __DIR__ . '/../app/bootstrap.php';

use SousMeow\Services\OutputContract;

$slugs = sm_money_collection_slugs();
$issues = [];
$matrix = [];
...
foreach ($slugs as $slug) {
    ...
    foreach ($book['recipes'] as $recipe) {
        ...
        if (!str_contains($template, 'Do not invent') && !str_contains($template, SM_MONEY_FACT_RULE)) {
            // Fact rule is interpolated via {$factRule} which expands SM_MONEY_FACT_RULE
        ...
        $banned = ['guaranteed approval', 'you will be approved', 'this will save you money', 'deserve a salary'];
...
$reportPath = __DIR__ . '/../research/MONEY_MAJOR_DECISIONS_QA.md';
$lines = [];
$lines[] = '# Money Collection QA';
$lines[] = '';
...
$lines[] = '## Seed status';
$lines[] = '';
$lines[] = 'Run `php scripts/seed.php --status` separately. Catalog health must report OK with 31 executable + 2 preview.';
$lines[] = '';
